...
abstract class TweetSet {
  ...
  def hasMoreRetweets(tweet: Tweet): Boolean
  ...
  def descList(tweets: TweetSet): TweetList
...
class Empty extends TweetSet {
  ...
  def hasMoreRetweets(tweet: Tweet): Boolean = false

  def descList(tweets: TweetSet): TweetList = {
    println("In empty descList")
    Nil
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (!restOfTweets.hasMoreRetweets(elem))
      elem
    else
      restOfTweets.mostRetweeted
  }

  def hasMoreRetweets(tweet: Tweet): Boolean = {
    elem.retweets > tweet.retweets || left.hasMoreRetweets(tweet) || right.hasMoreRetweets(tweet)
  }

  def descList(tweets: TweetSet): TweetList = {
    try {
      val most = tweets.mostRetweeted
      new Cons(most, descList(tweets.remove(most)))
    } catch {
